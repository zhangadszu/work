...
#删除旧的数据文件
os.system("rm /usr/local/services/xxx-1.0/bin/data/nrt_data_* -f")

# 打开数据文件
yesterdayDataTime = (datetime.date.today() - datetime.timedelta(days=1)).strftime("%Y%m%d") 
filePath = "/usr/local/services/xxx-1.0/bin/data/nrt_data_%s" % yesterdayDataTime
nrtDataFile = open(filePath, "w")

# 打开数据库连接
db = MySQLdb.connect(host="xxx",port=xxx,user="xxx",passwd="xxx",db="xxx",charset="xxx");
...
# NRT 上报
cmd = "/usr/local/services/xxx-1.0/bin/nrt_tool -i nrt141049 -t %s -f %s -c" % (yesterdayDataTime, filePath)
